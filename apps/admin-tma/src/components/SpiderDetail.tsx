...
import { X, Play, Power, Terminal, Calendar, Package, TrendingUp, Loader2, ChevronRight, Clock, AlertCircle, CheckCircle2, List, Filter, Settings2 } from "lucide-react";
...
export function SpiderDetail({ sourceId, onClose, onForceRun, onToggleActive, onUpdateSource, onOpenSource, isForceRunning, isUpdatingSource }: SpiderDetailProps) {
    ...
    const runHistory = source.history || [];
    const aggregateHistory = source.aggregate_history || [];
    const trendData = aggregateHistory.length > 0
        ? [...aggregateHistory].reverse()
        : [...runHistory]
            .map((h: any) => ({
                ...h,
                date: h.created_at,
                ...
                <div className="p-5 border-b border-[var(--tg-theme-secondary-bg-color)] flex items-center justify-between bg-gradient-to-r from-[var(--tg-theme-secondary-bg-color)] to-[var(--tg-theme-bg-color)]">
                    <div>
                        ...
                        <div className="mt-1 flex items-center gap-1.5 flex-wrap">
                            ...
                            {source.aggregate_history?.length > 0 ? (
                            ...
                            {trendData.length > 0 && (
                                <div className="space-y-3">
                                    <h3 className="text-sm font-bold flex items-center gap-2 px-1">
                                        ...
                                        {aggregateHistory.length > 0 ? "Aggregate Site Trend (daily)" : "Run Trend (latest runs)"}
                                    </h3>
