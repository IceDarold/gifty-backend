...
class TelegramInternalClient:
    ...
    async def get_subscriber(self, chat_id: int):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/{chat_id}"
    ...
    async def get_subscriber_by_username(self, username: str):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/by-username/{username}"
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, headers=self.headers)
            if resp.status_code == 200:
                return resp.json()
    ...
    async def create_subscriber(self, chat_id: int, name: str = None, slug: str = None):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers"
        async with httpx.AsyncClient() as client:
    ...
    async def create_invite(
        ...
        url = f"{self.api_base}/api/v1/internal/telegram/invites"
        payload = {
            "username": username,
            "password": password,
            "name": name,
    ...
    async def claim_invite(self, username: str, password: str, chat_id: int, name: str = None):
        url = f"{self.api_base}/api/v1/internal/telegram/invites/claim"
    ...
    async def set_role(self, chat_id: int, role: str):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/{chat_id}/role"
        async with httpx.AsyncClient() as client:
    ...
    async def set_permissions(self, chat_id: int, perms: list[str]):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/{chat_id}/permissions"
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, json=perms, headers=self.headers)
            return resp.status_code == 200

    async def subscribe(self, chat_id: int, topic: str):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/{chat_id}/subscribe"
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, params={"topic": topic}, headers=self.headers)
            return resp.status_code == 200

    async def unsubscribe(self, chat_id: int, topic: str):
        url = f"{self.api_base}/api/v1/internal/telegram/subscribers/{chat_id}/unsubscribe"
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, params={"topic": topic}, headers=self.headers)
            return resp.status_code == 200

    async def get_topic_subscribers(self, topic: str):
        url = f"{self.api_base}/api/v1/internal/telegram/topics/{topic}/subscribers"
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, headers=self.headers)
            if resp.status_code == 200:
                return resp.json()
    ...
    async def force_run_source(self, source_id: int, strategy: str = None):
        url = f"{self.api_base}/api/v1/internal/sources/{source_id}/force-run"
    ...
    async def get_active_workers(self):
        url = f"{self.api_base}/api/v1/internal/workers"
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, headers=self.headers)
            return resp.json() if resp.status_code == 200 else []
    ...
    async def connect_weeek(self, chat_id: int, token: str):
        url = f"{self.api_base}/api/v1/internal/weeek/connect"
    ...
    async def get_tasks(self, chat_id: int, type: str = "all", project_id: int = None):
        url = f"{self.api_base}/api/v1/internal/weeek/tasks"
    ...
    async def reschedule_task(self, task_id: int, chat_id: int, new_date: str, reason: str):
        url = f"{self.api_base}/api/v1/internal/weeek/tasks/{task_id}/reschedule"
        data = {
            "telegram_chat_id": chat_id,
            "new_date": new_date,
            "reason": reason
