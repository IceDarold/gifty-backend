...
class GoldAppleSpider(GiftyBaseSpider):
    ...
    def start_requests(self):
        if not self.url:
            self.url = "https://goldapple.ru/" if self.strategy == "discovery" else "https://goldapple.ru/makijazh"

        self.logger.info("Initializing GoldApple session via Playwright...")
    ...
    async def parse_rendered(self, response):
        ...
        if page:
            ...
            if self.strategy == "discovery":
                for item in self.parse_discovery(rendered_response):
                    yield item
            else:
    ...
    def parse_discovery(self, response):
        ...
        for link in links:
            ...
            yield self.create_category(url=url, name=name, parent_url=response.url)
    ...
    def parse_catalog(self, response):
        ...
        if self.strategy == "deep":
            yield from self._paginate(response, state)
    ...
    def _paginate(self, response, state):
        ...
        if current_page < last_page:
            ...
            next_url = f"{response.url.split('?')[0]}?p={next_page}"
            yield scrapy.Request(
                next_url,
                callback=self.parse_rendered,
                meta={
